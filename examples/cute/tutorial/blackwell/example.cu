...
__global__ void embrassingly_parallel(int4 *a, int4 *b, int4 *c,
                                      size_t n_fields) {
  for (auto i = blockIdx.x * blockDim.x + threadIdx.x; i < n_fields;
...
void throughput_test() {
  ...
  cudaEventCreate(&ee);
  cudaEventRecord(es);
  embrassingly_parallel<<<160 * 3, 512>>>(a, b, c, n_fields);
  cudaEventRecord(ee);
